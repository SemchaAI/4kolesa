...
void FaBoPWM::set_channel_value(uint8_t channel, uint16_t value){
  ...
  writeI2c(PCA9685_LED0_OFF_L_REG+channel*4, (value & 0xFF)); //Берем только младший байт
  writeI2c(PCA9685_LED0_OFF_H_REG+channel*4, (value >> 8)); //Берем только старший  байт
}
